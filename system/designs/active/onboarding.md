...
9. Dispatch an end-to-end developer task and confirm the PR opens
   ...
   logs, or audit rows.
10. Re-run the [tenant-isolation](./tenant-isolation.md) harness
...
- **Isolation is a CI-blocking gate, not a release note.**
  ...
  cross-bleed (see [tenant-isolation](./tenant-isolation.md)).
...
- **HTTP:** `POST /v1/projects`, `GET /v1/projects` (visibility
  ...
  project-scoped per [multi-tenancy](./multi-tenancy.md).
...
- Designs: [multi-tenancy](./multi-tenancy.md),
  [service-accounts](./service-accounts.md),
