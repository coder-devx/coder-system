...
### New files (shipped in c992a7b)

- `coder-core/src/coder_core/escalations/{__init__,detectors,
  escalation_policies.yaml,models,oncall,policies,watch,watcher}.py`
- `coder-core/src/coder_core/escalations/dispatchers/{__init__,
  pagerduty,slack}.py`
- `coder-core/src/coder_core/api/{escalations,on_call,slack_hooks}.py`
- `coder-core/src/coder_core/integrations/{slack_signing,slack_web}.py`
- `coder-core/src/coder_core/domain/{escalation,on_call_schedule}.py`
- `coder-core/migrations/versions/{0046_escalations,
  0047_on_call_schedules,0048_projects_escalation_columns}.py`
- `coder-admin/src/admin/escalations/*` + project tab

### Modified files

- `api/projects.py` — `PATCH /v1/projects/{id}` accepts SLA + policy
  + Slack channel + PD routing-key fields.
- `audit.py` — five new `escalation.*` actions in `Actions`.
- `api/metrics.py` — `escalations` block rollup.
...
## Rollout (per-ADR-0015 stages)

1. **Stage 1 — shadow (on ship).** Migrations applied; watcher
   deployed with `CODER_ESCALATIONS_ENABLED=false`. Scan queries
   log `escalation.would_open` events without writing rows.
2. **Stage 2 — L0 only, fleet.** Flip fleet flag; cap every policy
   at L0 via config override to tune thresholds on real stalls
   without paging anyone.
3. **Stage 3 — per-project full ladder.** Opt each project in via
   `escalation_policy=standard|aggressive` once the on-call schedule,
   Slack channel, and PD routing key are populated. `coder` first.
4. **Stage 4 — 0042 integration.** Self-healing watchdog calls
   `.../resolve` with `self_healing` actor; capture-rate metric
   reported.

## Backout

- `CODER_ESCALATIONS_ENABLED=false` — watcher short-circuits.
- `escalation_policy='off'` per project — next tick respects.
- Point `escalation_slack_channel_id` to a throwaway while tuning
  noise — zero code change.
- Tables + columns + the Cloud Run Job can be dropped wholesale
  at major-version cleanup if abandoned; the downgrade paths in
  0046/0047/0048 are intentionally destructive so you don't partial-
  roll-back the feature.

## Open questions

- **PagerDuty ack → our ack.** v1 doesn't round-trip. Wiring a
  PagerDuty webhook adds one hook endpoint + user mapping. Revisit
  when the rung-2 rate justifies the round trip.
- **Slack user ID on `users`.** v1 stores `acknowledged_by_type=
  'slack_external'` with the raw handle. Adding a column is a
  phase-2 polish once admin auth revisits external identities.
