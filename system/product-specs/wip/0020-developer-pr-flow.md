...
- [ ] AC1: Developer worker creates a branch named `task/{task_id}`, commits
  all changes, and pushes to the remote. The branch is visible on GitHub after
  the task completes.
- [ ] AC2: Developer worker opens a PR via `gh pr create` with a structured
  title and body. The PR URL appears in the worker's result text.
- [ ] AC3: The dispatcher extracts the PR URL from the developer result and
  stores it in `tasks.pr_url`. The field appears in `TaskRead` responses.
- [ ] AC4: When the orchestrator dispatches the reviewing stage, the reviewer's
  prompt includes the PR URL so the reviewer can `gh pr diff` and
  `gh pr review` against the real PR.
- [ ] AC5: When the reviewer approves, the PR is ready to merge via the
  existing `POST /tasks/{id}/merge` endpoint (spec 0012 AC5).
- [ ] AC6: If the developer worker fails to push or create a PR (e.g. auth
  error, conflict), the task fails with a clear error message rather than
  silently succeeding with no artifact.
- [ ] AC7: Migration 0016 adds `pr_url` (nullable String 500) to the tasks
